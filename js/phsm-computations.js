let PHSM = (function () {
	...
	return {
		...
		getBuyPreview: function (price, money) {
			...
			let	grossAmount = PHSM.getGrossAmount(price, maxVolume);
			let	totalCost = PHSM.getTotalBuyingCost(grossAmount);
		...
		getTotalBuyingCost: function () {
			...
			if (arguments.length === 1) {
				...
				const	buyingFees = PHSM.getBuyingFees(grossAmount);
			...
			// if two arguments was passed, we'll use it as stock price and volume
				...
				const grossAmount = PHSM.getGrossAmount(stockPrice, volume);
				const buyingFees = PHSM.getBuyingFees(grossAmount);
				totalBuyingCost = grossAmount + buyingFees.totalFees;
			}
			//}
		...
		getTotalSellingNet: function () {
			...
			if (arguments.length === 1) {
				const grossAmount = arguments[0]; // set the first argument as gross amount
				const	sellingFees = PHSM.getSellingFees(grossAmount);
		...
		getAverage: function () {
			...
			// it means it's just a single transaction
				...
				const stockPrice = arguments[0]; // set the first argument as stock price
				const	volume = arguments[1]; // set the second argument as volume
				const	totalCost = PHSM.getTotalBuyingCost(stockPrice, volume);
